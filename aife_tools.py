...
from aife_utils import retrieve, REMOVE_FILETYPE, manage_futures, upload_to_container, upload_and_email_file, resize_images, pdf_to_images, extract_text_with_xml_tag, extract_text_with_pattern, filter_words, now_and_choices, get_recent_dates_iso, iso_date
...
def relevant_search_results_to_csv(csv_path, search_target):
    ...
    if user_messages := [json.dumps(chunk[["title", "summary"]].to_dict("index"), indent=4, ensure_ascii=False) for chunk in chunks]:
        for attempt in range(3):
            try:
                llm_results = text_chats(["deepseek_v3_hyperbolic", "deepseek_v3_openrouter"], system_message, user_messages)
                if search_results := [search_result for i, indices in llm_results.items() for search_result in chunks[i].loc[chunks[i].index.isin(json.loads(indices))].to_dict("index").values()]:
                    ...
                    df = pd.DataFrame([{"web_text": web_texts[search_result["url"]], **{column: search_result.get(column) for column in columns if column in search_result}} for search_result in search_results if web_texts[search_result["url"]]]).reindex(columns=columns)
                    df.to_csv(csv_path, index=False, encoding="utf-8")
                    return True
            except Exception:
                continue
...
def accepted_search_results_to_csv(csv_path, search_target, acceptance_criteria):
    ...
    df = pd.read_csv(csv_path, encoding="utf-8")
    if web_texts := [row["web_text"] for i, row in df.iterrows() if pd.notna(row["web_text"])]:
        for attempt in range(3):
            try:
                llm_results = text_chats(["gemini2_flash_openrouter"], system_message, web_texts)
                if search_results := [df.iloc[i].to_dict() for i in range(len(web_texts)) if (llm_result := llm_results[i]) and "False" not in llm_result and "false" not in llm_result]:
                    columns = ["search_target", "url", "title", "web_text", "date"]
