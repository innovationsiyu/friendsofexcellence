...
from scraper import tidy_body_contents, get_web_contents, get_web_texts, get_web_screenshots
...
from aife_utils import retrieve, manage_thread, upload_to_container, upload_and_email_file, resize_images, pdf_to_images, extract_text_with_xml_tag, extract_text_with_tags, extract_text_with_pattern, filter_words, now_and_choices, get_recent_dates_iso, iso_date
...
def request_llm(url, headers, data):
    for attempt in range(2):
        try:
            ...
            response = requests.post(url, headers=headers, json=data, timeout=150).json()
            print(response)
            if message := response.get("choices", [{}])[0].get("message", {}):
                if tool_calls := message.get("tool_calls"):
                    ...
                    if reasoning_content := message.get("reasoning_content") or message.get("reasoning"):
...
def route_llm_request(configs, messages, temperature=None, tools=None):
    for config in configs:
        try:
            llm_config = llm_configs[config]
            name = llm_config["name"]
            arguments = {**llm_config["arguments"], "temperature": temperature} if temperature else llm_config["arguments"]
...
def text_chat(configs, system_message, user_message, index=None):
...
def text_chats(configs, system_message, user_messages):
    ...
    requests = [(text_chat, (configs[i % len(configs)], system_message, user_message, i)) for i, user_message in enumerate(user_messages if isinstance(user_messages, list) else [user_messages], start=0) if user_message]
...
jobs = {
    "Text chat": {
        "category": "text",
        "llms": ["DeepSeek-R1", "QwQ 32B", "DeepSeek-V3", "Qwen-Max", "MiniMax-01", "Gemini Flash 2.0", "Phi 4", "Claude 3.7 Sonnet", "o3 mini high", "GPT-4o", "Grok 2", "Yi-Lightning", "LFM 7B"],
    ...
    "Multimodal chat with tools": {
        "category": "multimodal",
        "llms": ["MiniMax-01", "Gemini Flash 2.0", "GPT-4o", "Claude 3.7 Sonnet"],
    ...
    "Logic checking": {
        "category": "text",
        "llms": ["DeepSeek-R1", "QwQ 32B", "Claude 3.7 Sonnet", "o3 mini high"],
...
llms = {
    ...
    "DeepSeek-V3": {
        "configs": ["deepseek_v3_hyperbolic", "deepseek_v3", "deepseek_v3_openrouter"],
        "context_length": 65536
    },
    "DeepSeek-R1 Search": {
    ...
    "gpt4o_mini": {
        "configs": ["gpt4o_mini_excellence", "gpt4o_mini_openrouter"],
        "context_length": 128000
    },
    "qwen_vl_plus": {
        "configs": ["qwen25_vl_7b_dashscope", "qwen_vl_plus_dashscope"],
        "context_length": None
    },
    "qwen_vl_max": {
        "configs": ["qwen25_vl_72b_dashscope", "qwen_vl_max_dashscope"],
        "context_length": None
    }
}
...
search_results_to_word_func = {
    "type": "function",
    "function": {
        "name": "search_results_to_word",
        "description": "Read the CSV file containing the search results, export it to a Word document, and return a URL for downloading it.",
        "parameters": {
            "type": "object",
            "properties": {
                "csv_path": {
...
def search_results_to_csv(csv_path, search_target, search_results, freshness=None):
    columns = ["search_target", "title", "summary", "publication_info", "url", "date"]
    ...
    if freshness:
        df = df[df["date"].isin(get_recent_dates_iso(freshness))]
...
def relevant_search_results_to_csv(csv_path, search_target):
    ...
    if user_messages := [json.dumps(chunk[["title", "summary"]].to_dict("index"), indent=4, ensure_ascii=False) for chunk in chunks]:
        for attempt in range(3):
            try:
                llm_results = {i: json.loads(llm_result) for i, llm_result in text_chats(["deepseek_v3_hyperbolic", "deepseek_v3_openrouter"], system_message, user_messages).items()}
                if search_results := [search_result for i, indices in llm_results.items() for search_result in chunks[i].loc[chunks[i].index.isin(indices)].to_dict("index").values()]:
                    urls = [search_result["url"] for search_result in search_results]
                    web_screenshots = get_web_screenshots(urls)
                    columns = ["search_target", "title", "web_screenshot", "publication_info", "url", "date"]
                    df = pd.DataFrame([{"web_screenshot": web_screenshots[search_result["url"]], **{column: search_result.get(column) for column in columns if column in search_result}} for search_result in search_results if web_screenshots[search_result["url"]]]).reindex(columns=columns)
                    df.to_csv(csv_path, index=False, encoding="utf-8")
                    return True
            except Exception:
                continue
...
def accepted_search_results_to_csv(csv_path, search_target, acceptance_criteria):
    user_message = get_prompt("is_acceptable_search_result", search_target=search_target, acceptance_criteria=acceptance_criteria)
    df = pd.read_csv(csv_path, encoding="utf-8")
    if image_paths := [row["web_screenshot"] for i, row in df.iterrows() if pd.notna(row["web_screenshot"])]:
        for attempt in range(3):
            try:
                llm_results = image_chats(["gemini2_flash_openrouter"], user_message, image_paths)
                if search_results := [df.iloc[i].to_dict() for i, image_path in enumerate(image_paths) if (llm_result := llm_results[image_path]) and "False" not in llm_result and "false" not in llm_result]:
                    columns = ["search_target", "title", "web_screenshot", "publication_info", "url", "date"]
...
def get_structured_web_info(llm_result, web_content):
    if structured_web_info := extract_text_with_tags(extract_text_with_xml_tag(llm_result, "structured_info"), ["Title", "Source", "Published date", "First paragraph", "Last paragraph"]):
        return {"doc_title": structured_web_info["Title"], "source": structured_web_info["Source"], "published_date": iso_date(structured_web_info["Published date"]), "body_content": extract_body_content(web_content, structured_web_info["First paragraph"], structured_web_info["Last paragraph"])}
    return None


def structured_search_results_to_csv(csv_path):
    user_message = get_prompt("extract_structured_web_info")
    df = pd.read_csv(csv_path, encoding="utf-8")
    if image_paths := [row["web_screenshot"] for i, row in df.iterrows() if pd.notna(row["web_screenshot"])]:
        for attempt in range(3):
            try:
                urls = [row["url"] for i, row in df.iterrows() if pd.notna(row["web_screenshot"])]
                web_contents = get_web_contents(urls)
                llm_results = image_chats(["gemini2_flash_openrouter"], user_message, image_paths)
                if search_results := [{**df.iloc[i].to_dict(), **structured_web_info} for i, image_path in enumerate(image_paths) if (llm_result := llm_results[image_path]) and len(llm_result) > 100 and (web_content := web_contents[df.iloc[i]["url"]]) and (structured_web_info := get_structured_web_info(llm_result, web_content))]:
                    columns = ["search_target", "title", "web_screenshot", "doc_title", "source", "published_date", "body_content", "publication_info", "url", "date"]
                    df = pd.DataFrame([{column: search_result.get(column) for column in columns if column in search_result} for search_result in search_results]).reindex(columns=columns)
                    df.to_csv(csv_path, index=False, encoding="utf-8")
                    return True
            except Exception:
...
def aggregate_search_results(csv_path, existing_search_results=None):
    df = pd.read_csv(csv_path, encoding="utf-8")
...
def scholar_search_by_year_range(search_target, acceptance_criteria, query, year_range=None, existing_search_results=None):
    requests = [
        (google_scholar_by_year_range, (query, year_range)),
        (exa_paper_by_year_range, (query, year_range))
    ]
    if search_results := [item for result, function, arguments in manage_thread(requests) if result for item in result]:
        ...
        if relevant_search_results_to_csv(csv_path, search_target):
            if accepted_search_results_to_csv(csv_path, search_target, acceptance_criteria):
                if structured_search_results_to_csv(csv_path):
                    return aggregate_search_results(csv_path, existing_search_results)
    return None


def scholar_search_by_freshness(search_target, acceptance_criteria, query, freshness=None, existing_search_results=None):
    requests = [
        (exa_paper_by_freshness, (query, freshness))
    ]
    if search_results := [item for result, function, arguments in manage_thread(requests) if result for item in result]:
        csv_path = f"temp-data/{sanitize_filename(search_target[:20])} {now_and_choices()}.csv"
        search_results_to_csv(csv_path, search_target, search_results, freshness)
        if relevant_search_results_to_csv(csv_path, search_target):
            if accepted_search_results_to_csv(csv_path, search_target, acceptance_criteria):
                if structured_search_results_to_csv(csv_path):
                    return aggregate_search_results(csv_path, existing_search_results)
...
def endeavour_search(search_target, acceptance_criteria, required_quantity, query, freshness_or_year_range, system_message, existing_search_results=None, retry=0):
    ...
    if search_results and len(search_results) >= required_quantity or retry == 2:
        ...
        for attempt in range(3):
            try:
                llm_result = text_chat(["gemini2_flash_openrouter"], system_message, user_message)
...
def search_results_to_excel(search_results):
    df = pd.DataFrame(search_results)
    df = df.drop_duplicates(subset="url", keep="first")
    search_results = df.to_dict("records")
    image_paths = [search_result["web_screenshot"] for search_result in search_results if search_result.get("web_screenshot") and os.path.isfile(search_result["web_screenshot"])]
    resize_images(image_paths, 720, True)
    workbook = Workbook()
    worksheet = workbook.active
    worksheet.title = "Search Results"
    columns = list(search_results[0].keys())
    for column_index, column in enumerate(columns, start=1):
        worksheet.cell(row=1, column=column_index, value=column)
    for row_index, search_result in enumerate(search_results, start=2):
        for column_index, column in enumerate(columns, start=1):
            cell_value = search_result.get(column)
            cell = worksheet.cell(row=row_index, column=column_index)
            if column == "web_screenshot" and cell_value and os.path.isfile(cell_value):
                worksheet.add_image(Image(cell_value), f"{get_column_letter(column_index)}{row_index}")
            else:
                cell.value = cell_value
                cell.alignment = Alignment(wrap_text=True, vertical="top")
    for column_index, column in enumerate(columns, start=1):
        column_letter = get_column_letter(column_index)
        if column in ["web_screenshot", "body_content"]:
            worksheet.column_dimensions[column_letter].width = 720 / 8
        else:
            worksheet.column_dimensions[column_letter].width = 240 / 8
    for row_index in range(2, len(search_results) + 2):
        worksheet.row_dimensions[row_index].height = 720 / 1.333
    xls_path = f"temp-data/{now_and_choices()}.xlsx"
...
def information_retrieval(search_strategies):
    ...
    if search_results := [item for result, function, arguments in manage_thread(requests) if result for item in result]:
        xls_path = search_results_to_excel(search_results)
        return json.dumps({"url_to_download_all_results": upload_to_container(xls_path)}, indent=4, ensure_ascii=False)
    return None


def ensure_structured_search_results(csv_path):
    df = pd.read_csv(csv_path, encoding="utf-8")
    df = df[df["doc_title"].notna() & df["source"].notna() & df["published_date"].notna() & df["body_content"].notna()]
    if manual_indices := [i for i in df.index if not (df.at[i, "body_content"].strip().startswith('[') and df.at[i, "body_content"].strip().endswith(']'))]:
        ...
        for i in manual_indices:
            df.at[i, "body_content"] = processed_body_contents[df.at[i, "body_content"]]
            df.at[i, "published_date"] = iso_date(df.at[i, "published_date"])
    df.to_csv(csv_path, index=False, encoding="utf-8")
    return True


def search_results_to_word(csv_path):
...
def describe_pages(pdf_path):
    if image_paths := pdf_to_images(pdf_path):
        if any(image_paths):
            ...
            llm_results = image_chats(["qwen25_vl_7b_dashscope", "qwen_vl_plus_dashscope"], get_prompt("is_plain_text"), image_paths)
            ...
            if any(image_paths):
                llm_results = image_chats(["qwen25_vl_72b_dashscope", "qwen_vl_max_dashscope"], get_prompt("describe_visual_elements"), image_paths)
...
def process_csv_content(csv_path, column_to_process, column_of_results, system_message):
    ...
    results = text_chats(["phi4_deepinfra", "phi4_openrouter"], system_message, user_messages)
...
def information_for_audio_interpretation(doc_contents, outline):
    ...
    results = text_chats(["gpt4o_mini_openrouter", "gpt4o_mini_excellence"], system_message, user_messages)
    return {i: extract_text_with_pattern(extract_text_with_xml_tag(results[i], "information_for_questions"), r"Information for Question \d+:") for i in results}
                                                ...
                                                if audios and sum(1 for audio in audios.values() if not audio) / len(audios) < 0.2:
                                                    for attempt in range(3):
                                                        try:
                                                            ...
                                                            if wav_path:
                                                                file_url = upload_and_email_file(wav_path, to_email)
                                                                return file_url
                                                        except Exception:
