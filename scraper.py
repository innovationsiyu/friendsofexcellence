...
def get_image(image_url):
    try:
        with Image.open(BytesIO(requests.get(image_url, timeout=(30, 180)).content)) as f:
            if max(f.size) < 100:
                return None
            if min(f.size) > 1024:
        ...
        print(f"Error in get_image from {image_url}: {e}")
        return None


def get_images(image_urls):
    requests = [(get_image, (image_url,)) for image_url in (image_urls if isinstance(image_urls, list) else [image_urls])]
    return [result for result, function, arguments in manage_futures(requests)]


def insert_image_paths(lines):
    if image_url_indices := [i for i, line in enumerate(lines) if url(line)]:
        image_urls = [lines[i] for i in image_url_indices]
        image_paths = get_images(image_urls)
        for i, image_path in zip(image_url_indices, image_paths):
            if image_path:
                lines[i] = image_path
    return lines
