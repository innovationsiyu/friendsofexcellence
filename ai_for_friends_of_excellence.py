...
st.session_state["answer_language"] = st.session_state.get("answer_language", "Chinese (simplified)")
...
def should_use_chinese():
    return st.session_state["answer_language"] == "Chinese (simplified)" or st.session_state["answer_language"] == "Chinese (traditional)"
...
def display_messages(index=0):
    messages = st.session_state["chat_history"][index:]
...
file_name_and_path_tuples = get_file_name_and_path_tuples([file for file in files if file.name not in {file_name for message in st.session_state["chat_history"] if "file_names" in message for file_name in message["file_names"]}])
...
with st.sidebar:
    ...
    with st.container(height=(window_height - 352) // 2, border=False):
    ...
    with st.expander("More Settings", expanded=False):
        ...
        answer_languages = ["Chinese (simplified)", "Chinese (traditional)", "English (American)", "English (British)"]
        st.pills("Answer in (default)", answer_languages, key="answer_language")

        pdf_types = ["Plain Text", "With Visuals"]
        st.pills("Type of PDF to read", pdf_types, key="pdf_type")
...
if user_message:
    ...
    if st.session_state["edit_mode"]:
        st.text_area("Edit messages", height=window_height - 278, key="chat_history_editable", on_change=is_chat_history_edited, label_visibility="collapsed")
    else:
        display_messages()
