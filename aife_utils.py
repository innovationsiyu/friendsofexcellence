    ...
    for attempt in range(3):
        try:
            ...
            with open(file_path, "rb") as file, mmap.mmap(file.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                ...
                if all(result[0] for result in manage_thread(requests)):
...
def ensure_utf8_csvs(table_paths):
    ...
    for table_path in table_paths:
        if table_path:
            ...
            requests.append(request)
    return [result for result, function, arguments in manage_thread(requests) if result]
...
def now_and_choices():
    return now().strftime("%Y%m%d%H%M%S") + "".join(choices(digits + ascii_lowercase, k=4))
...
def get_present_day_en():
    return f"{now().day}{get_day_suffix(now().day)}"


def get_present_day_zh():
    return f"{now().day}日"
...
def get_present_weekday_en():
    return weekdays_en[now().weekday()]


def get_present_weekday_zh():
...
def get_present_month_en():
    return months_en[now().month - 1]


def get_present_month_zh():
...
def get_today_with_weekday_en():
    present = now()
    return f"{months_en[present.month - 1]} {present.day}{get_day_suffix(present.day)}, {weekdays_en[present.weekday()]}"


def get_today_with_weekday_zh():
    present = now()
    return f"{present.month}月{present.day}日，星期{weekdays_zh[present.weekday()]}"
...
def get_weekdays_with_dates_en():
    present = now()
    week_start = present - timedelta(days=present.weekday())
...
def year_start(year):
    return (datetime(year, 1, 1, tzinfo=ZoneInfo("Asia/Shanghai")) if year <= now().year else datetime(now().year, 1, 1, tzinfo=ZoneInfo("Asia/Shanghai")))


def year_end(year):
    return (datetime(year, 12, 31, 23, 59, 59, 999999, tzinfo=ZoneInfo("Asia/Shanghai")) if year < now().year else now())
