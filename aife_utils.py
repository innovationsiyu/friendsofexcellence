...
def extract_text_with_tags(text, tags):
    ...
    result = {}
    for tag in tags:
        pattern = r"(?i)" + rf"{tag}:\s*(?P<result>[\s\S]*?)(?=\s*(?:{'|'.join(tag + ':' for tag in tags)}|$))"
        if matched := re.search(pattern, text):
            result[tag] = matched.group("result").strip()
    return result
...
def extract_text_with_pattern(text, pattern):
    if markers := [(marker.start(), marker.group()) for marker in re.finditer(pattern, text)]:
        return [text[marker[0] + len(marker[1]):markers[i + 1][0] if i < len(markers) - 1 else len(text)].strip() for i, marker in enumerate(markers)]
    return None
